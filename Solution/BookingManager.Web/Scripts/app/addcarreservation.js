        ...
        success: function (data) {
            if (data.Success) {
                if (data.CarReservationId == -1) {
                    location.href = "/CarReservations";
                }
                else {
                    location.href = "/CarReservations/Edit?Id=" + data.CarReservationId;
                }
            }
            else {
                toastr.clear();
                toastr.error('Ocurrió un error intentando adicionar esta reserva. ' + data.ErrorDescription, 'Oops !!!')
            }
        }
    });
}

PageController.prototype.onSaveAndStay = function () {
    $('#SaveAndStay').val('true');
    submitForm();
};

PageController.prototype.onSaveAndReturn = function () {
...
PageController.prototype.onCarProviderChanged = function () {
    ...
    if (carproviderid == "") carproviderid = -1;
